...
byte servoPin_claw = 27; // Assigning pin 26 to main claw
byte servoPin_rotate = 26; // Asigning pin 27 to servo 1
...
void setup() 
{
  ...
  int clawSignal = 700;
  servo_rotate.writeMicroseconds(clawSignal);

  int rotateSignal = 700; // range from 900 to 1700
...
void loop() 
{
  ...
  if(Serial.available()) 
  {
    ...
    //Vertical Thruster 1 
    float th_up_left = json_doc["tup_left"];
    int th_up_sig_1=(th_up_left+1)*400+1100; //map controller to servo
    servo_up1.writeMicroseconds(th_up_sig_1); //pin 25

    //Vertical Thruster 2
    float th_up_right = json_doc["tup_right"];
    int th_up_sig_2=(th_up_right+1)*400+1100; //map controller to servo
    servo_up2.writeMicroseconds(th_up_sig_2); //Send signal to ESC
    
    //Main Claw Code
    int clawInput = json_doc["claw"]; // inputs range from 0 to 100
    int clawSignal = clawInput*10+700; // range from 900 to 1700
    ...
    int rotateSignal = rotateInput*10+700; // range from 600 to 2400 
